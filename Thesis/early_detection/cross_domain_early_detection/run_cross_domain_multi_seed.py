...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the matched cross-domain early-detection RF/MLP configurations across multiple seeds."
        )
    )
    parser.add_argument(
        "--iot_data_dir",
    ...
    parser.add_argument(
        ...
        help="Directory where seed-specific run folders will be written.",
    )
    parser.add_argument(
        "--directions",
        nargs="+",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Cap on IoT-23 source-train rows for IoT-23 -> UNSW-NB15.",
    )
    parser.add_argument(
        "--unsw_train_max_rows",
        type=int,
        default=100000,
        help="Cap on UNSW-NB15 source-train rows for UNSW-NB15 -> IoT-23.",
    )
    parser.add_argument(
        "--iot_eval_max_rows_per_scenario",
        type=int,
        default=50000,
        help="Cap on earliest IoT-23 evaluation rows retained per scenario.",
    )
    parser.add_argument(
        "--unsw_eval_max_rows",
        type=int,
        default=30000,
        help="Cap on earliest UNSW-NB15 validation and test rows retained for evaluation.",
    )
    parser.add_argument(
        "--rf_n_estimators",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Python interpreter used to launch the per-seed runs.",
    )
    parser.add_argument(
        "--overwrite_existing",
        action="store_true",
    ...
    parser.add_argument(
        "--continue_on_error",
        action="store_true",
        help="Continue with later runs if one seed/model/direction fails.",
    )
    parser.add_argument(
        "--dry_run",
        action="store_true",
...
def build_run_name(direction: str, model: str, seed: int) -> str:
    return f"outputs_{direction}_{model}_seed{seed}"
...
def build_command(
    ...
    if model == "rf":
        runner_path = thesis_root / "early_detection" / "cross_domain_early_detection" / "run_cross_domain_early_detection.py"
    else:
        runner_path = thesis_root / "early_detection" / "cross_domain_early_detection" / "run_cross_domain_mlp_early_detection.py"
    ...
    if direction == "iot23_to_unsw":
        if args.iot_train_max_rows is not None:
            command.extend(["--iot_train_max_rows", str(args.iot_train_max_rows)])
        if args.unsw_eval_max_rows is not None:
            command.extend(["--unsw_eval_max_rows", str(args.unsw_eval_max_rows)])
    else:
        if args.unsw_train_max_rows is not None:
            command.extend(["--unsw_train_max_rows", str(args.unsw_train_max_rows)])
        if args.iot_eval_max_rows_per_scenario is not None:
            command.extend(["--iot_eval_max_rows_per_scenario", str(args.iot_eval_max_rows_per_scenario)])
...
def main() -> None:
    ...
    for direction in args.directions:
        for model in args.models:
            for seed in args.seeds:
                run_name = build_run_name(direction, model, seed)
                run_out_dir = out_dir / run_name
                config_path = run_out_dir / "run_config.json"
                command = build_command(thesis_root, args, direction, model, seed, run_out_dir)
                planned_runs.append(
                    {
                        "run_name": run_name,
                        "direction": direction,
                ...
                try:
                    ...
                    if not args.continue_on_error:
                        save_json(
                            ...
                            out_dir / "multi_seed_run_manifest.json",
                        )
                        raise
    ...
    if failures:
        raise SystemExit(f"Completed with {len(failures)} failed runs. See multi_seed_run_manifest.json.")
