...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Cross-domain early-detection RF experiments between IoT-23 and UNSW-NB15 "
    ...
    parser.add_argument(
        "--out_dir",
        default="early_detection/cross_domain_early_detection/outputs_rf_exp1",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Random Forest parallel jobs. Default 1 for safer local execution.",
    )
...
def main() -> None:
    ...
    for direction_name in directions_to_run:
        ...
        joblib.dump(pipeline, direction_dir / "rf_pipeline.joblib")
        pd.DataFrame({"feature": feature_cols}).to_csv(direction_dir / "used_features.csv", index=False)
        save_feature_importance(pipeline, feature_cols, direction_dir / "feature_importance.csv")
        ...
        if target_type == "unsw":
            ...
            all_unsw_details.append(add_detail_metadata(val_results["details"], direction_name, "val", "rf"))
            all_unsw_details.append(add_detail_metadata(test_results["details"], direction_name, "test", "rf"))
        else:
            ...
            all_iot_details.append(add_detail_metadata(val_results["details"], direction_name, "val", "rf"))
            all_iot_details.append(add_detail_metadata(test_results["details"], direction_name, "test", "rf"))
