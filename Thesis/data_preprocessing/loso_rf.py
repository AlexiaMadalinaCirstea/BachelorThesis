...
"""
LOSO runner for the Random Forest IoT-23 baseline.

Why this script exists:
- reuse generate_loso_splits(...) from splitters.py
- reuse RF pipeline + feature definitions from train_baseline_rf.py
- save predictions in the exact format expected by evaluate.py
- keep experiment orchestration separate from model/evaluation code

Example:
    python run_loso_rf.py \
        --data_file "Datasets\\IoT23\\processed_test_sample\\iot23\\all_flows.parquet" \
        --out_dir "Datasets\\IoT23\\processed_test_sample\\iot23\\rf_loso" \
...
def run_fold(df: pd.DataFrame, split: Dict[str, List[str]], out_dir: Path, target_col: str, seed: int) -> Dict[str, object]:
    ...
    if train_df.empty:
        raise AssertionError(f"{fold_name}: train_df is empty")
    if test_df.empty:
        raise AssertionError(f"{fold_name}: test_df is empty")

    log.info(
        "[%s] train scenarios=%d rows=%d | test scenario=%s rows=%d",
        fold_name,
        len(train_scenarios),
        len(train_df),
        test_scenario,
    ...
    log.info("[%s] train label distribution:\n%s", fold_name, train_df[target_col].value_counts(dropna=False).to_string())
    log.info("[%s] test label distribution:\n%s", fold_name, test_df[target_col].value_counts(dropna=False).to_string())
    ...
    pred_df = make_predictions_df(test_df=test_df, y_pred=y_pred, y_score=y_score, target_col=target_col)
    pred_path = fold_dir / "rf_test_predictions.parquet"
    ...
    log.info(
        "[%s] accuracy=%.4f f1_macro=%.4f recall_attack=%.4f",
        fold_name,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Leave-One-Scenario-Out RF baseline on IoT-23")
